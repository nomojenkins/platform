...
- `viewType`
    ...
    - `GRID`
        ...
        Ключевое слово, при указании которого выбирается вид представления *колонка таблицы*. Используется по умолчанию.
    ...
    - `MAP [tileProvider]` 
        ...
        При указании ключевого слова `MAP` выбирается вид представления *карта*.

        - `tileProvider` - строковый литерал, указывающий источник карт. Возможные варианты: `'google'` и `'yandex'`. Если не указывается, то источником карт будет являться OpenStreetMap.
...
[//]: # (      )
[//]: # (            Строковый литерал, который определяет имя функции, возвращающей рендерер.)

[//]: # ()
[//]: # (        - `expr`)

[//]: # (      )
[//]: # (            Выражение, значение которого должно являться json с данными, не зависящими от объектов.)
