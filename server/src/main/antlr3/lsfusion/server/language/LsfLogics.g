...
@after {
	if (inMainParseState()) {
		$property = self.addScriptedJSONProperty(context, $plist.aliases, $plist.literals, $plist.properties, $whereExpr.property, orderProperties, orderDirections, returnString);
	}
}
...
@after {
	...
	: 	('CLASS' | 'ISCLASS') '(' expr=propertyExpression[context, dynamic] ')'
	;
...
primitiveType returns [String val]
	:	p=PRIMITIVE_TYPE | JSON_TYPE | JSON_TEXT_TYPE { $val = $p.text; }
	;
